...
# only secret arn to be queried for AWSPENDING stage label locally on ec2
#potential to have lambda query for success flag (SMTPPASSWORDUPDATESUCCESSFUL) in stdout
...
##do stuff here to update application with new values, check for errors
RESULT=$(echo $SMTP_USER >> /tmp/smtp_user)

#only put specific flag into send_command stdout if successful; for lambda search
if [ $RESULT -eq 0 ]; then
